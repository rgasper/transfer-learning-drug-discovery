"""Run CheMeleon CV training with disk caching.

Trains CheMeleon foundation model variants (single-finetune and
double-finetune) across all folds and saves predictions to disk.

Two variants:
1. Single-finetune: CheMeleon weights -> finetune on target endpoint
2. Double-finetune: CheMeleon weights -> finetune on RLM -> finetune on target
...
CACHE_DIR = DATA_DIR / "chemeleon_cache"
...
def download_chemeleon() -> None:
    """Download CheMeleon foundation model weights from Zenodo."""
    if CHEMELEON_PATH.exists():
        logger.info(f"CheMeleon weights already at {CHEMELEON_PATH}")
        return
    logger.info(f"Downloading CheMeleon from {CHEMELEON_URL}")
    urlretrieve(CHEMELEON_URL, CHEMELEON_PATH)
    logger.info(f"Saved to {CHEMELEON_PATH}")
...
def build_chemeleon_model() -> models.MPNN:
    """Build a CheMeleon MPNN for binary classification."""
...
def main() -> None:
    ...
    # --- CheMeleon double-finetune: first finetune on RLM ---
    rlm_chemeleon_checkpoint = CHECKPOINTS_DIR / "chemeleon_rlm_finetuned.ckpt"
    if not rlm_chemeleon_checkpoint.exists():
        logger.info("Finetuning CheMeleon on RLM...")
        ...
        rlm_model = build_chemeleon_model()
        trainer = lightning_pl.Trainer(
            logger=False,
            enable_checkpointing=False,
            enable_progress_bar=False,
        ...
        trainer.save_checkpoint(rlm_chemeleon_checkpoint)
        logger.info(f"Saved CheMeleon-RLM checkpoint to {rlm_chemeleon_checkpoint}")
    else:
        logger.info(
            f"Using existing CheMeleon-RLM checkpoint at {rlm_chemeleon_checkpoint}"
        )
        ...
        for rep in range(n_replicates):
            ...
            for fold in range(n_folds):
                ...
                if both_cached:
                    ...
                    # --- Single finetune: CheMeleon -> target ---
                    if cache_path_single.exists():
                        y_prob_single = np.load(cache_path_single)["y_prob"]
                        n_cached += 1
                    else:
                        model_single = build_chemeleon_model()
                    ...
                    # --- Double finetune: CheMeleon -> RLM -> target ---
                    if cache_path_double.exists():
                        y_prob_double = np.load(cache_path_double)["y_prob"]
                        n_cached += 1
                    else:
                        model_double = models.MPNN.load_from_file(
                            rlm_chemeleon_checkpoint
                        )
                ...
                all_results.append(
                    {
                        "target": target_name,
                        "model": "CheMeleon double-finetune",
                        "replicate": rep,
                        "fold": fold,
                        **metrics_double,
                    }
                ...
                logger.info(
                    ...
                    f"single AUC={metrics_single['auc_roc']:.3f}, "
                    f"double AUC={metrics_double['auc_roc']:.3f}"
    ...
    results_df.write_parquet(DATA_DIR / "chemeleon_results.parquet")
    logger.info(
        f"Saved {results_df.height} results to {DATA_DIR / 'chemeleon_results.parquet'}"
    )
