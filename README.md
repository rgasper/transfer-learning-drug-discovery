...
This document (and codebase) explores a practical question: does transfer learning help for ADME prediction (yes), and does it matter what model type you use and how you perform that knowlege transfer (yes), and why?

The bottom line: graph neural networks with representation-level transfer learning are the only approach we tested that is both safe and productive. The end result of using that is that we can train models on new data-poor endpoints with dramatically better performance than starting new models from scratch. When the source and target tasks share biochemistry, transfer learning significantly improves predictions; when they don't, the model gracefully ignores the irrelevant pre-training and performs as well as training from scratch. XGBoost's standard transfer protocol cannot make this guarantee -- it can catastrophically fail on unrelated sources, collapsing to random-chance performance. Foundation models (large neural networks pre-trained on millions of compounds) go further still: a frozen foundation encoder matches or exceeds task-specific models at every training set size above ~200 compounds, and reaches full XGBoost performance with only 25% of the data. For drug discovery teams, the practical takeaway is that investing in neural network infrastructure and foundation model pre-training pays off not because these models score a few points higher on a leaderboard, but because they make transfer learning *safe to use by default* and they dramatically reduce the amount of expensive experimental data needed to build useful models. On these specific datasets and models, we proved that representation-level transfer never makes things worse (disproving "more pre-training can hurt" for D-MPNNs), and we proved that decision-boundary transfer *can* make things catastrophically worse. We did not prove that representation-level transfer always helps -- on unrelated targets, it was indistinguishable from training from scratch.
