...
CheMeleon tells the same story from a different starting point. Its encoder was pre-trained on 1M compounds for physicochemical property prediction -- no biological endpoints involved -- so its learned molecular vocabulary is inherently task-agnostic. On HLM, the additional RLM finetuning step provides a modest but statistically significant gain: double-finetune (Foundation→RLM→HLM) improves over single-finetune (Foundation→HLM) by +0.016 AUC-PR. The frozen variants (where only the FFN head trains on top of the locked foundation encoder) perform even better, sitting in the top statistical group alongside Chemprop RLM-transfer. The pattern is consistent with Chemprop's: representation-level transfer from a related source helps, and a richer starting vocabulary (from foundation pre-training) provides a stronger baseline to build on.
...
CheMeleon's frozen encoder provides the cleanest test of this principle. Since the encoder weights are locked, any difference between single-finetune (Foundation→PAMPA) and double-finetune (Foundation→RLM→PAMPA) can only come from the FFN head -- the molecular representations themselves are identical. And indeed, on PAMPA the two frozen configurations are statistically indistinguishable (0.921 vs 0.922 AUC-PR). The feature importance rankings confirm why:
...
Independently trained FFN heads converge on the same atomic attention pattern -- sulfur environments dominate in both, followed by aromatic oxygens and nitrogens. The irrelevant RLM intermediate step does not corrupt the foundation's vocabulary because the encoder is never modified; the new FFN head simply learns the PAMPA mapping from the same stable feature set regardless of what task preceded it. This is representation-level transfer taken to its logical conclusion: the richer and more general the learned features, the less the model cares what path you took to get there. (The foundation model also initially underperformed due to overfitting when fully finetuned; freezing the encoder fixed this -- see [docs/foundation-model-puzzle.md](docs/foundation-model-puzzle.md) for the full investigation.)
...
From the very beginning, we advertised that transfer learning is used because it allows you to get effective models with less training data. So, is that true for these datasets? To test how the architectures compare with less data, we trained all three on 1%, 10%, 25%, 50%, 75%, and 100% of the training data for each endpoint (subsampling within each CV fold, 25 folds per fraction). We also added a second foundation model to the comparison: **MIST** (28M parameters, mist-models/mist-28M-ti624ev1), a SMILES transformer pre-trained on Enamine REAL Space using masked language modeling. Like CheMeleon, MIST is evaluated frozen with a Chemprop-shaped FFN head trained on top of cached encoder embeddings.
...
At 1% (~20 compounds), all four architectures cluster near the random baseline -- there's simply not enough data for any model to learn from. But the curves diverge rapidly. By 10% (~200 compounds), CheMeleon frozen already leads significantly, and by 25% (~500 compounds) it matches XGBoost's 100%-data performance -- a result stable across all 25 CV folds. XGBoost initially keeps pace with Chemprop but plateaus around 50%, while Chemprop continues climbing and overtakes XGBoost at the 50% mark.

MIST is the more interesting addition. At low data fractions (1-25%) it is competitive with the best models, often statistically indistinguishable from CheMeleon. But unlike CheMeleon, MIST plateaus by 50% on every endpoint and never closes the gap with CheMeleon at higher data fractions. By 100%, MIST sits roughly with XGBoost (RLM: MIST 0.477 vs XGBoost 0.487 vs CheMeleon 0.585; HLM: MIST 0.752 vs XGBoost 0.745 vs CheMeleon 0.823). We don't have the experiments needed to claim *why* the two foundation models behave so differently on these endpoints. The two encoders differ along several axes at once: training corpus (Enamine REAL synthesis library vs. PubChem), pre-training objective (masked language modeling vs. supervised physicochemical property prediction), architecture (SMILES transformer vs. D-MPNN), and embedding dimensionality (512 vs. 2048). Any of these could explain the divergence and we have not run the controlled experiments needed to attribute it. A reasonable guess is that an objective involving real chemical properties produces representations more useful for downstream property prediction than one that only predicts structural tokens, but this is not something we have tested. The result we *can* defend is empirical: the choice of foundation model matters, and "frozen foundation encoder + small head" is not a reliable shortcut on its own. To rule out that MIST's plateau was caused by the small 1-layer head being too narrow a bottleneck, we ran a separate experiment with a 2-layer FFN head (one extra Linear+ReLU block) on top of both frozen encoders. The deeper head produced no consistent improvement for either encoder on any endpoint -- head capacity is not the bottleneck. The plot is at `docs/figures/data-efficiency-deep-head.png` for anyone who wants to see it.

We also ran the same frozen-encoder protocol with MIST-1.8B (mist-models/mist-1.8B-dh61satt, 1.8B parameters, hidden_size=2304). The result: statistically indistinguishable from MIST-28M at every data fraction on all three endpoints. This is informative because it rules out embedding dimensionality as the explanation for the MIST/CheMeleon gap -- the 1.8B model's [CLS] vector (2304-d) is actually *wider* than CheMeleon's pooled representation (2048-d), yet carries no additional predictive value for these tasks. The most parsimonious explanation is that the pre-training objective is the binding constraint: masked language modeling teaches the encoder to reconstruct SMILES tokens, which does not require compressing property-relevant chemical information into the pooled representation. Scaling the encoder 64x gives it lower perplexity on held-out SMILES but does not change what information the [CLS] token encodes. We omit MIST-1.8B from the main data-efficiency plot to avoid visual clutter since its curve overlaps MIST-28M entirely.

The practical implication is narrower than we originally claimed: when the foundation model's pre-training is well-matched to the downstream task, frozen embeddings + a small head can match or beat full-data conventional models with a fraction of the labeled data. When it isn't, you may end up with a frozen model performing on par with XGBoost. There's no obvious way to predict which class your foundation model falls into without running this experiment. CheMeleon happened to work for ADME prediction; we don't know in advance which other foundation models will.

A caveat on the MIST results: we used MIST in a configuration its authors do not recommend. The [official MIST finetuning tutorial](https://github.com/BattModels/mist-demo/blob/main/tutorials/run_finetuning.ipynb) trains the encoder end-to-end with LoRA adapters on the attention layers plus a 2-layer MLP head, with per-task hyperparameter search via Optuna over learning rate, dropout, head depth, and batch size. The 400+ property-prediction benchmarks the MIST paper reports were produced with that recipe, not with a frozen encoder feeding a fixed Chemprop-shaped head. We chose the frozen-encoder protocol here for apples-to-apples comparison with CheMeleon (which itself performs better frozen than fine-tuned on these endpoints, see [docs/foundation-model-puzzle.md](docs/foundation-model-puzzle.md)). It is plausible that MIST + LoRA finetuning + per-endpoint hyperparameter tuning would close some or all of the gap with CheMeleon. Running that experiment is left as future work; on a back-of-envelope basis it would require GPU and a per-fold finetuning loop rather than a cached-embedding sweep, so it's an order of magnitude more expensive than what we ran here. The current MIST numbers should be read as "frozen MIST embeddings + simple head" rather than "MIST as the authors intend it to be used."
...
3. **Data efficiency, when the foundation model fits.** As shown [above](#data-efficiency), CheMeleon frozen matches full-data XGBoost performance with only 25% of the training data on all three endpoints. The practical implication: if you have fewer than ~500 task-specific compounds and a foundation model whose pre-training is well-matched to your task, frozen embeddings + a small head can substantially outperform what XGBoost or a scratch D-MPNN can do with the same data. The MIST results are an important caveat -- a different foundation model with different pre-training (SMILES transformer, masked language modeling on a synthesis library) plateaus alongside XGBoost on these endpoints. We can't tell from this experiment which axis of difference between MIST and CheMeleon (corpus, objective, architecture, embedding size) is responsible. So the recommendation is qualified: foundation models *can* deliver large data-efficiency wins, but the choice of foundation model matters and you'd want to validate fit on a representative held-out set before committing.

4. **Pre-trained representations can encode real chemistry, sometimes.** CheMeleon frozen dominates at every data fraction from 10% onward on these endpoints, producing representations competitive with task-specific Chemprop training -- without any task-specific message-passing gradients. As shown in the [PAMPA feature importance analysis](#what-the-models-learn-how-transfer-changes-feature-importance), independently trained FFN heads on top of the frozen CheMeleon encoder converge on the same atomic attention pattern regardless of whether the model was routed through RLM first. That's a strong piece of evidence that CheMeleon's representations encode something stable and chemistry-relevant. The MIST results show this is not automatic: another foundation model trained on a different corpus with a different objective produced representations that cap out alongside XGBoost on these tasks. We don't yet know what makes a foundation model's representations useful for ADME prediction in particular, and the field as a whole is still figuring out how to evaluate molecular foundation models beyond benchmark-set leaderboards. The current results suggest that pre-trained representations are a real lever -- but pulling the wrong one gets you a frozen model that performs like a much simpler one.
...
Transfer learning works for molecular property prediction, but the choice of *transfer protocol* determines both whether it's safe and whether it's useful. We tested three points on the transfer spectrum: XGBoost's standard continue-boosting protocol transfers decision boundaries -- helpful when the source and target are related (+0.059 on HLM), catastrophic when they're not (-0.057 on PAMPA), and irrecoverable either way. An alternative XGBoost protocol transferring only feature-importance priors avoids the catastrophic failure but also provides no benefit on either endpoint -- it is safe but inert. D-MPNN representation transfer is the only protocol that is both safe and beneficial: neutral on unrelated targets (indistinguishable from scratch, though the saliency analysis shows the encoder substantively changes the model's internal feature landscape without degrading performance), and significantly helpful on related ones (+0.038 on HLM), because the encoder's learned molecular vocabulary provides compositional feature structure that a binary feature-selection prior cannot. Foundation models like CheMeleon can take this further when their pre-training fits the downstream task: a frozen CheMeleon encoder provides representations that outperform task-specific training at every data size above 10% on these ADME endpoints, with feature importance patterns that are stable across independently trained decision heads. The MIST results temper this -- a different foundation model on the same setup performed comparably to XGBoost rather than dominating it -- so foundation-model gains aren't automatic and depend on which model you pick. For teams building ADME prediction pipelines, the practical recommendation is: use XGBoost for quick, interpretable single-endpoint models when you have enough data and no transfer ambitions; use a D-MPNN when you want to build on prior work across endpoints, because it is the only architecture that offers both safe and productive transfer; and consider a frozen foundation model when data is scarce, after validating that its representations actually help on a representative slice of your problem.
